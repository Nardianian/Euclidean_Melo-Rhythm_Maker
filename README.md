...
C) The MIDI stream for each rhythm is generated based on various independent commands or rotary knobs, which in order on the GUI are:
   ...
   C8) Note Len (Note Length): to set the duration of the MIDI note (with a different programming for the sixth rhythm).
...
D) ARP, the MIDI stream thus generated enters the arpeggiator:
   ...
   D3) Using the tickable popup menu with "ARP Notes" snapshot, user can view the incoming notes (from the MIDI stream of the rhythm created so far) and can select up to seven notes with their respective octaves (e.g., 
         C-1, G#-7, etc.), simultaneously from all the notes received from the MIDI stream. Only the selected notes will be arpeggiated. Unselected notes continue unchanged, adding to the arpeggiated notes. Notes 
         generated by the rhythm but not selected in the arp behave as if the arp were not activated for those notes.
   ...
   D5) Use "ARP Rate" to select the arpeggio speed of the selected notes.
...
E) Finally, the rhythm thus created, including any possible arpeggio (if activate on some Rhythm channel), is output from a MIDI-Out whose port and channel can be selected: the goal of having independent outputs for 
   ...
   a drum kit and a bass (on the sixth rhythm, which has a different note length), but the possibility of implementing options suitable for melodic instruments is being studied.
